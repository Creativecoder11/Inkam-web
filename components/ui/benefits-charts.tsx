...
export default function BenefitsCharts() {
  ...
  const maxValue = 25;
  const maxHeight = 480; // px height of tallest bar
  const yAxisLabels = ["0", "5M", "10M", "15M", "20M", "25M"];

  return (
    <div ref={chartRef} className="bg-[#161518] rounded-xl text-white p-8 font-sans">
      <div className="max-w-6xl mx-auto">

        {/* Header */}
        <div className="flex justify-between items-center">
          <div className="mb-12">
            <h1 className="text-3xl font-bold mb-2">Financial Projection</h1>
            <p className="text-gray-400 text-base">Monitor your monthly income growth</p>
          </div>
        ...
        <div className="relative">
            ...
            {yAxisLabels
              ...
              .map((label, i) => (
                <div key={i} className="relative">
                  <span className="absolute text-white text-base left-6 -translate-y-1/2">{label}</span>
                </div>
              ))}
            <div className="absolute flex items-center text-white/50 text-[14px] -left-6 top-1/2 -rotate-90 italic">
              <p>Value</p>
          ...
          <div className="ml-16 relative" style={{ height: "400px" }}>
            ...
            {/* Bars */}
            <div className="absolute  left-0 right-0 flex items-end justify-between h-full pl-1">
              {data.map((item, index) => {
                const barHeight = animated ? (item.value / maxValue) * maxHeight : 0;

                return (
                  <div key={index} className="flex flex-col items-center flex-1 max-w-11 relative">
                    ...
                    <div className="w-full flex justify-center mb-3 relative">
                      ...
                      {/* Bar */}
                      <div
                        className="w-20 bg-(--orange) rounded-t-lg transition-all duration-1000 ease-out"
                        style={{
                          height: `${barHeight}px`,
                        }}
                      ></div>
                      ...
                      {item.showLabel && (
                        ...
                        >
                          <div className="p-3 rounded-full bg-black text-center">
                            <div className="text-sm font-bold">{item.label}</div>
                            <div className="text-[12px] text-gray-400">Growth</div>
                          </div>
                        </div>
                      )}
                    ...
                    {/* Year Label */}
                    <div className="text-gray-300 bg-[#1a1a1a] text-base font-medium">
                      {item.year}
                    </div>
                  </div>
                );
